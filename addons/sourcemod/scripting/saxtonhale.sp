...
//  RegAdminCmd("hale_eggs", Command_Eggs, ADMFLAG_ROOT);   //WILL CRASH.
    ...
    LogMessage("===Versus Saxton Hale Initializing - v%s===", PLUGIN_VERSION);
    cvarVersion = CreateConVar("hale_version", PLUGIN_VERSION, "VS Saxton Hale Version", FCVAR_NOTIFY|FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_DONTRECORD);
...
public OnConfigsExecuted()
{
    ...
    if (strcmp(oldversion, PLUGIN_VERSION, false) != 0) LogError("[VS Saxton Hale] Warning: your config may be outdated. Back up your tf/cfg/sourcemod/SaxtonHale.cfg file and delete it, and this plugin will generate a new one that you can then modify to your original values.");
    SetConVarString(FindConVar("hale_version"), PLUGIN_VERSION);
    ...
    if (IsSaxtonHaleMap() && GetConVarBool(cvarEnabled))
    {
        ...
        if (steamtools)
        {
            ...
            Format(gameDesc, sizeof(gameDesc), "VS Saxton Hale (%s)", PLUGIN_VERSION);
...
public Action:Timer_Announce(Handle:hTimer)
{
    ...
    if (Announce > 1.0 && g_bAreEnoughPlayersPlaying)
    {
        switch (announcecount)
        {
            ...
            case 3:
            {
                CPrintToChatAll("{default}VSH v%s by {olive}Rainbolt Dash{default}, {olive}FlaminSarge{default}, & {lightsteelblue}Chdata{default}.", PLUGIN_VERSION);
            }
            case 5:
            {
...
public Action:NewPanel(client, versionindex)
{
    ...
    new Handle:panel = CreatePanel();
    decl String:s[90], String:s2[90], String:s3[90];
    GetVersionTitle(versionindex, s2, sizeof(s2));
    SetGlobalTransTarget(client);
    Format(s, 90, "=%t%s:=", "vsh_whatsnew", s2);
    SetPanelTitle(panel, s);
    ...
    {
        GetVersionTitle(versionindex, s2, sizeof(s2));
        GetVersionTitle(versionindex-1, s3, sizeof(s3));
        if (strcmp(s2, s3, false) == 0)
        {
            Format(s, 90, "Next Page");
        }
        else
        {
            Format(s, 90, "Older v%s", s3); 
        }
        DrawPanelItem(panel, s);
    }
    else
